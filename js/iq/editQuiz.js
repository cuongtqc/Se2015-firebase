app.controller( 'UpdateIQ' , function( $scope , $firebaseArray , $firebaseObject ) {
	// initiate value
	$scope.editDB = true;
	$scope.loaded = false;
	$scope.addTab = false;
	...
	var IQ = new Firebase("https://se15.firebaseio.com/iq");
	var bank = $firebaseArray( IQ );  
	bank.$loaded (
		function( data ) {
			$scope.list = data;
			$scope.temp = data;
	...
	$scope.deleteQuiz = function( index ) {
		...
		if ( r ) {
			var list = [];
			IQ.on('value', function(snap) { list = snap.val(); });
			list.splice(index, 1);
			IQ.set(list);
		};
	}
